...
export default function ExamPrep() {
  ...
  const handleDownloadQuestions = async (format: 'txt' | 'pdf' = 'txt') => {
    ...
    try {
      if (format === 'pdf') {
        ...
        const response = await fetch(`/api/download-questions?format=pdf`, {
          method: 'GET',
        });
        ...
        a.href = url;
        a.download = `important_questions_${new Date().toISOString().split('T')[0]}.pdf`;
      ...
      setError(`Failed to download questions as ${format.toUpperCase()}`);
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <Button 
                      onClick={() => handleDownloadQuestions('txt')} 
                      className="w-full"
                      variant="outline"
                    >
                      <Download className="h-4 w-4 mr-2" />
