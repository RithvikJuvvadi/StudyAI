...
Focus on finding actual questions in the text along with their answers.

Return a JSON array of objects with these fields:
- question: string
- answer: string (or "Answer not provided" if not found)
- importance: "high" | "medium" | "low"
- topic: string
- difficulty: "easy" | "medium" | "hard"
...
${content}`;
    ...
    const response = await fetch(GROQ_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            ...
            content: 'You are an expert at extracting and generating educational questions from documents. ' +
                     ...
                     'Return only the questions that are explicitly present in the text.'
          },
          {
            role: 'user',
            content: prompt
        ...
        temperature: 0.3,
        max_tokens: 4000, // Reduced since we're chunking
      }),
    });
...
const selectImportantQuestions = (allQuestions: Array<{
  ...
  // Select top questions from each category
  const selectedQuestions = [
    ...highImportance.slice(0, 8), // Top 8 high importance
    ...mediumImportance.slice(0, 5) // Top 5 medium importance
  ];
...
export const handleGenerateQuestions: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    console.log(`Calling Python backend at: ${pythonBackendUrl}/api/generate-questions`);
...
export const handleDownloadQuestions: RequestHandler = (req, res) => {
  try {
    ...
    const { format = 'pdf', includeAnswers = true, includeTopics = true } = req.query;

    // Find the questions
    let questions = generatedQuestions.find(q => q.id === questionsId);
    ...
    if (format === 'pdf' || format === 'txt') {
      content = generateTextContent(questions, includeAnswers as boolean, includeTopics as boolean, timestamp);
    } else if (format === 'docx') {
      content = generateDocxContent(questions, includeAnswers as boolean, includeTopics as boolean, timestamp);
    }

    // Set headers based on format
    const filename = `important_questions_${questions.id}.${format}`;
