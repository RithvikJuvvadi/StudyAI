...
4. GENERATE ANSWERS: For each question, you MUST generate a comprehensive answer based on the document content.
   CRITICAL: Answer length MUST be proportional to question importance - THIS IS MANDATORY:
   ...
   Answer guidelines:
   - For multiple choice questions: Provide the correct option letter (A, B, C, D, etc.) AND a detailed explanation spanning 1-3 paragraphs based on importance
   - For open-ended questions: Provide thorough explanations spanning the appropriate number of paragraphs (1-3 based on importance)
   - For fill-in-the-blank questions: Provide the missing word/phrase and a detailed explanation (1-2 paragraphs)
   - Answers should be well-structured, informative, accurate, and based solely on the document content provided
   - If the document doesn't contain enough information, infer a reasonable answer based on the context
   - NEVER use "Answer not provided" - always generate a comprehensive answer
   - IMPORTANT: Each paragraph should be substantial (3-5 sentences minimum). Do NOT create single-sentence paragraphs.

5. Answer format examples:
   - Multiple choice (HIGH importance): "C. [2-3 paragraph detailed explanation of why C is correct, covering all relevant aspects from the document]"
   - Multiple choice (MEDIUM importance): "C. [1-2 paragraph explanation of why C is correct based on document]"
   - Open-ended (HIGH importance): "[2-3 paragraphs: comprehensive explanation covering all aspects of the concept based on document content]"
   - Open-ended (MEDIUM/LOW importance): "[1-2 paragraphs or 1 paragraph: detailed but appropriately scoped explanation]"
   - Fill-in-the-blank: "[Missing word/phrase]. [1-2 paragraph explanation]"

Return a JSON array of objects with these fields:
- question: string (COMPLETE question with ALL options if present - this is critical!)
- answer: string (GENERATED comprehensive answer - 1 to 3 paragraphs based on importance: HIGH=2-3 paragraphs, MEDIUM=1-2 paragraphs, LOW=1 paragraph. NEVER use "Answer not provided")
- importance: "high" | "medium" | "low" (determines answer length: high=2-3 paragraphs, medium=1-2 paragraphs, low=1 paragraph)
- topic: string
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    'temperature': 0.3,
                    'max_tokens': 10000  # Increased to allow for longer answers (1-3 paragraphs based on importance)
                },
                timeout=60
            )
